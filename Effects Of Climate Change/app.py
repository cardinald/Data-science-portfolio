...
def load_data(data_file: str) -> pd.DataFrame:
    ...
    try:
        ...
        try:
            ...
            ...
            print
            "thanks for your CSV file"
        except:  # if you find specific Exception types, use them here
            print
            "sorry, now way, give me some usable file."


# Load data

df = load_data("idmc_disaster_all_dataset.csv")
dfs = load_data("emdat.xlsx")
df_temp = load_data("temperature.csv")

#df1 = pd.read_csv('data/idmc_disaster_all_dataset.csv')
#df2 = pd.read_excel('data/emdat.xlsx')
...
sidebar = html.Div(
    [
        html.Div([html.H1("The Effects of Climate Change", className="display-4", style={"margin-left": 9.5})],
                 style={"background": "#000039", "padding": "0.5rem 0.5rem", "font-size": 15}),
...
page_1_layout = html.Div([
    ...
    # navigation header
    html.Div([
        dcc.Link('Population Displacements', href='/page-1', style={'background': "#ddd",
                                                                    "color": "black"}),
        dcc.Link('Natural Disasters', href='/page-2'),
    ], className="navbar"),

    # page content
    html.Div([
        html.Div([
            dcc.Graph(id='map1', style={'height': '100%', 'width': '100%'}),
        ], style={'width': '100%', 'height': '55vh', 'overflow-x': 'auto',
                  }),

        html.Div([
            dcc.Graph(id='graph2', style={'height': '100%', 'width': '100%'}),
        ], style={'width': '50%', 'display': 'inline-block', 'height': '45vh', 'overflow-x': 'auto',
                  }),
...
page_2_layout = html.Div([
    ...
    html.Div([
        dcc.Link('Population Displacements', href='/page-1'),
        dcc.Link('Natural Disasters', href='/page-2', style={'background': "#ddd",
                                                             "color": "black"}),
    ], className="navbar"),
...
@app.callback(dash.dependencies.Output('g1', 'figure'),
              ...
              dash.dependencies.Input('dis-checklist1', 'value'),
              dash.dependencies.Input('dis_country_dropdown', 'value'))
def update_map2(selected_year, selected_disaster, selected_country):
    filtered_df = dfs[dfs.Year == selected_year]
    if selected_disaster and selected_country:
        # filter by disaster type
        filtered_dis = filtered_df[filtered_df['Disaster Type'].isin(selected_disaster)]
        # filter by selected countries
        filtered_df3 = filtered_dis[filtered_dis.Country == selected_country]

    elif selected_country and not selected_disaster:
        filtered_df3 = filtered_df[filtered_df.Country == selected_country]
    ...
    if filtered_df2.empty:
        map_fig = px.scatter_geo(filtered_df, locations="ISO")
        map_fig.update_traces(marker=dict(size=0, opacity=0, ))
    ...
    map_fig.update_geos(
        ...
        showocean=True, oceancolor="#2E3033",
        showlakes=True, lakecolor="#202945",
        showcountries=True, countrycolor="#3D3D3D",

        # fitbounds="locations" - zooms on location
    )
...
def update_map2(selected_year, selected_disaster):
    ...
    map_fig = px.scatter_geo(filtered_df2, locations="ISO3", color="Hazard_Type",
                             color_discrete_map=color_map, title="Displacements",
                             size="New Displacements", size_max=70,
